...
Answer: the variables names are species, island, bill_length_mm, bill_depth_mm, flipper_length_mm, body_mass_g, sex and year. The impartant values are in the below table.
...
Answer: Numeric / logical vector works, but the character/factor vector doesn’t.
```{r}
mean(dataframe$samp)
...
Answer: After being converted to numeric vectors, the factor/ charactor/logical vectors  all work. When trying to take the mean, I need to make sure the vectors are numeric or logical ectors.
...
#mean
mean(dataframe2$gt10_logic)
mean(dataframe2$leg10_char)
mean(dataframe2$level_fact)
